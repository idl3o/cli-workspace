...
const { app, BrowserWindow, Menu, shell, dialog, Tray, ipcMain, nativeImage } = require('electron');
...
class QuantumConsciousnessApp {
    ...
    async onReady() {
        ...
        try {
            console.log('🚀 Starting Quantum Consciousness server...');
            
            // Use dynamic import for ES module
            const { default: DigitalServer } = await import('./digital-server.js');
            
            this.server = new DigitalServer({
                port: this.serverPort,
                publicPath: path.join(__dirname, 'webapp'),
            ...
            console.log(`✅ Server running on http://localhost:${this.serverPort}`);
                        ...
                        submenu: [
                            {
                                ...
                                click: () => this.activateComponent('sensory-gateway')
                            },
                            {
                                label: 'Emotional Gateway',
                                accelerator: 'CmdOrCtrl+6',
                                click: () => this.activateComponent('emotional-gateway')
                            },
                            {
                                label: 'Cognitive Gateway',
                                accelerator: 'CmdOrCtrl+7',
                                click: () => this.activateComponent('cognitive-gateway')
                            },
                            {
                                label: 'Transcendent Gateway',
                                accelerator: 'CmdOrCtrl+8',
                                click: () => this.activateComponent('transcendent-gateway')
                            }
                        ]
                    }
                ]
