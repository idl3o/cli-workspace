...
program
  ...
  .action(async () => {
    try {
      ...
      const errorConsciousness = new ErrorConsciousness()
      await errorConsciousness.handleError(error as Error, { 
...
program
  ...
  .action(async () => {
    try {
      ...
      // Intention prompt
      const intentionResponse = await consciousness.ask(ZenPromptFactory.intention(
        'What would you like to accomplish today?',
        ['Write code', 'Learn something new', 'Solve a problem', 'Explore ideas']
      ))
      
      console.log(chalk.green(`✨ Your intention: ${intentionResponse.value}`))
      
      // Flow prompt
      const flowResponse = await consciousness.ask(ZenPromptFactory.flow(
        'How would you prefer to work on this?',
        ['Step by step', 'Quick iteration', 'Deep focus', 'Collaborative exploration']
      ))
      
      console.log(chalk.blue(`🌊 Your flow: ${flowResponse.value}`))
      
      // Contemplation prompt
      const contemplationResponse = await consciousness.ask(ZenPromptFactory.contemplation(
  ...
  .action(async (errorMessage) => {
    try {
      ...
      if (errorMessage) {
        ...
        const response = await consciousness.ask(ZenPromptFactory.intention(
          'Describe an error or challenge you\'ve encountered recently'
        ))
